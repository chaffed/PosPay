...
def _submission_from_row(session: Session, tenant_id: uuid.UUID, row: dict[str, Any]) -> AchTransactionSubmission:
    ...
    transaction_type (debit/credit), sec_code, trace_number, effective_date."""
...
def _submission_from_nacha_entry(session: Session, tenant_id: uuid.UUID, entry: NachaEntry) -> AchTransactionSubmission:
...
def ingest_ach_rows(session: Session, tenant_id: uuid.UUID, rows: list[dict[str, Any]]) -> list[BulkFileRowResult]:
    ...
    for index, row in enumerate(rows):
        ...
        try:
            submission = _submission_from_row(session, tenant_id, row)
...
def ingest_nacha_entries(session: Session, tenant_id: uuid.UUID, entries: list[NachaEntry]) -> list[BulkFileRowResult]:
    ...
    for entry in entries:
        ...
        try:
            submission = _submission_from_nacha_entry(session, tenant_id, entry)
            txn = ingest_ach_transaction(session, tenant_id, submission)
            session.commit()
            results.append(BulkFileRowResult(row_label=row_label, success=True, created_id=txn.id))
        except Exception as exc:  # noqa: BLE001 — isolate row failures in a bulk file
